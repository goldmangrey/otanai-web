...
import { isTableBlockLanguage } from '../../utils/tableBlock.js'
import TableBlock from '../../../components/chat/TableBlock.jsx'
...
function CodeBlock({ className, children, documentContext = null, renderDocumentPreview = null }) {
  const [isCopied, setIsCopied] = useState(false)
  ...
  if (isDocumentLike(content, normalizedLanguage, documentContext)) {
    return typeof renderDocumentPreview === 'function'
...
export function createMarkdownComponents({ documentContext = null, renderDocumentPreview = null } = {}) {
  return {
    a: ({ href, children }) => {
      const hrefValue = safeUrl(href)
      if (!hrefValue) return <>{children}</>
      return (
